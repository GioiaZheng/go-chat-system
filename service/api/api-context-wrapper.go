...
var (
	...
	// Returned when the Authorization header exists but has an unexpected format.
...
func (rt *_router) wrap(fn httpRouterHandler) func(http.ResponseWriter, *http.Request, httprouter.Params) {
	return func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		// This adds a round trip but provides stronger guarantees.
...
func extractTokenFromHeader(r *http.Request) (string, error) {
	...
	// This allows simple clients that send only the token string.
	if raw == "" {
		return "", ErrNoToken
	}
	return raw, nil
